...
use crate::web2::common::io::Rewind;
use crate::web2::common::{task, Future, Pin, Poll};
...
pub struct OnUpgrade {
    rx: Option<oneshot::Receiver<crate::web2::error::Result<Upgraded>>>,
}
...
impl Future for OnUpgrade {
    type Output = Result<Upgraded, crate::web2::error::Error>;
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> Poll<Self::Output> {
        match self.rx {
            Some(ref mut rx) => Pin::new(rx).poll(cx).map(|res| match res {
                ...
                Ok(Err(err)) => Err(err),
                Err(_oneshot_canceled) => Err(crate::web2::error::Error::new_canceled().with(UpgradeExpected)),
            }),
            None => Poll::Ready(Err(crate::web2::error::Error::new_user_no_upgrade())),
        }
    }
}
